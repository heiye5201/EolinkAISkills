...
def _upsert_restful_param(case_data: dict, key: str, value: str) -> None:
    rest = list(case_data.get("restful_param") or [])
    for item in rest:
        if str(item.get("param_key", "")).lower() == key.lower():
            item["checkbox"] = True
            item["param_info"] = value
            item["param_value"] = value
            case_data["restful_param"] = rest
            return
    rest.append(
        {
            "checkbox": True,
...
def main():
    parser = argparse.ArgumentParser(description="在 Eolink API Studio 中创建/更新“编辑优惠码”测试用例（调整 adjustment_type）")
    parser.add_argument("--config", default="eolink_config.json", help="配置文件路径")
    parser.add_argument("--keyword", default="编辑优惠码", help="用于搜索接口的关键字")
    parser.add_argument("--api-id", type=int, default=None, help="直接指定 api_id（跳过搜索）")
    parser.add_argument("--case-name", default="编辑优惠码-自动生成", help="用例名称（单条用例）")
    parser.add_argument(
        "--adjustment-types",
    ...
    parser.add_argument("--restful-id-var", default="{{edit_coupon_id}}", help="restful path 变量值（如 coupon id）")
    ...
    def upsert_one(case_name: str, *, adjustment_type: Optional[str] = None) -> dict:
        ...
        case_data = case_info.get("case_data") or {}
        _upsert_restful_param(case_data, "id", args.restful_id_var)
        if adjustment_type:
    ...
    if args.create_adjustment_type_cases:
        ...
        if options:
            print("adjustment_type options:", json.dumps(options, ensure_ascii=False))
        for t in types:
