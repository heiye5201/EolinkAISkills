#!/usr/bin/env python3
"""
顶层入口：为 eolink-test skill 提供一条命令做三件事：
 1. 确保优惠码/编辑优惠码的 adjustment_type 用例存在
 2. 可选地再生成通知用例（可扩展）
 3. 调用 `run_eolink_test.py` 全量跑一遍匹配的接口
"""
...
def main():
    ...
    parser.add_argument("--coupon-adjustment-types", default="percentage,flat,m_for_n,buy_x_get_y_discount", help="传给创建优惠码脚本的 adjustment_type 列表")
    parser.add_argument("--edit-adjustment-types", default="percentage,flat,m_for_n,buy_x_get_y_discount", help="传给编辑优惠码脚本的 adjustment_type 列表")
    parser.add_argument("--edit-api-id", type=int, default=None, help="直接给编辑接口的 apiId（默认关键词搜素）")
    parser.add_argument("--restful-id-var", default="{{edit_coupon_id}}", help="写入编辑接口 restful path 的变量")
    parser.add_argument("--skip-tests", action="store_true", help="只生成用例，不执行 run_eolink_test")
    ...
    parser.add_argument("--output", default="eolink_test_report.json", help="run_eolink_test 的 output 参数")
    ...
    if args.skip_tests:
        print("⚙️  只生成用例，跳过 run_eolink_test.py")
        return
